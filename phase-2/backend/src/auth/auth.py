...
def get_current_user_id(
    ...
    Also verifies that the user exists and is active in the database
    """
    token = credentials.credentials
    ...
    # Verify the user exists in the database and is active
    user = session.get(User, token_data.user_id)
    if user is None:
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
    ...
    # Check if user is active (for email verification)
...
# Dependencies to be used in API routes
from fastapi import Depends

CurrentUser = Depends(get_current_user)
CurrentUserId = Depends(get_current_user_id)

# Helper functions for dynamic dependency creation (if needed)
