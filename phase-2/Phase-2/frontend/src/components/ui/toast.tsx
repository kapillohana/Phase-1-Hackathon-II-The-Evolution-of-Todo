...
const ToastItem = ({ toast: toastItem }: { toast: Toast }) => {
  const { icon: Icon, iconColor } = getTypeConfig(toastItem.type);
  ...
  return (
    ...
    >
      ...
      <div className="ml-3 flex-1">
        <p className="text-sm font-medium text-gray-900 dark:text-white">{toastItem.title}</p>
        {toastItem.description && (
          <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">{toastItem.description}</p>
        )}
      </div>
      <button
