...
Implements all 6 required endpoints with advanced features and authentication
"""
...
from ..auth.auth import CurrentUser, CurrentUserId, verify_user_id_match
...
from ..schemas.task_schemas import (
    ...
    TaskListQueryParams, TaskListResponse as TaskListResponseSchema, PaginationInfo
)
...
@app.post("/api/{user_id}/tasks", response_model=TaskPublic, status_code=status.HTTP_201_CREATED)
def create_new_task(
    user_id: int,
    task_create: TaskCreate,
    token_user_id: int = Depends(CurrentUserId),
...
@app.get("/api/{user_id}/tasks", response_model=TaskListResponseSchema)
def list_tasks(
    user_id: int,
    token_user_id: int = Depends(CurrentUserId),
    session: Session = Depends(get_session),
...
@app.get("/api/{user_id}/tasks/{task_id}", response_model=TaskPublic)
def get_single_task(
    user_id: int,
    task_id: int,
    token_user_id: int = Depends(CurrentUserId),
...
@app.put("/api/{user_id}/tasks/{task_id}", response_model=TaskPublic)
...
@app.delete("/api/{user_id}/tasks/{task_id}", status_code=status.HTTP_200_OK)
def remove_task(
    user_id: int,
    task_id: int,
    token_user_id: int = Depends(CurrentUserId),
...
@app.patch("/api/{user_id}/tasks/{task_id}/complete", response_model=dict)
